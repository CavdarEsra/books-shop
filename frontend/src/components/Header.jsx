    ...
    <header className='fixed top-0 w-full left-0 right-0 z-50'>
        <div className={`${active ? 'bg-white py-2.5' : 'bg-primary py-3'} max-padd-container flexBetween border-b border-slate-900/10 rounded transition-all duration-300`}>
            <Link to={'/'} className='flex-1 flex items-center justify-start'>
                ...
                <h4 className='bold-24'>ESRA BOOKS</h4>
            </Link>
            <div className='flex-1'>
                <Navbar menuOpened={menuOpened} toggleMenu={toggleMenu} containerStyles={`${menuOpened ? "flex flex-col gap-y-16 h-screen w-[222px] absolute left-0 top-0 bg-white z-50 px-10 py-4 shadow-xl" : "hidden xl:flex justify-center gap-x-8 xl:gap-x-14 medium-15 px-2py-1"}`}/>
            </div>
            <div className='flex-1 flex items-center justify-end gap-x-3 sm:gap-x-10'>
                ...
                <div className='relative group'>
                    <div onClick = {!token && navigate('/')}>
                        ...
                        {token && 
                            <>
                                <ul className='bg-white p-1 w-32 ring-1 ring-slate-900/10 rounded absolute right-0 top-7 hidden group-hover:flex flex-col regular-14 shadow-md'>
