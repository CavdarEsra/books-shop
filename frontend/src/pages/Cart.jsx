                      ...
                      <div className='flex items-start justify-between'>
                        <div>
                          ...
                          <div className='flex items-center ring-1 ring-slate-900/5 rounded-full overflow-hidden bg-primary'>
                            <button onClick={()=>updateQuantity(item._id, cartItems[item._id] - 1)} className='p-1.5 bg-white rounded-full shadow-md'><FaMinus className='text-sm'/></button>
                            <p className='px-2'>{cartItems[item._id]}</p>
                            <button onClick={()=>updateQuantity(item._id, cartItems[item._id] + 1)} className='p-1.5 bg-white rounded-full shadow-md'><FaPlus className='text-sm'/></button>
                          </div>
                        </div>
