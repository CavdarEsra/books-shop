...
function Shop() {
  ...
  // sadece bir kategory seçilebilir yap
  ...
  const applyFilters = () => {
    ...
    // Kategori filtresi - sadece seçili kategori varsa uygula
    if (category) {
      ...
      <div className="pt-28">
        ...
        <div className='mt-12 mb-16'>
          ...
          <div className='flexCenter sm:flexStart flex-wrap gap-x-12 gap-y-4'>
            {categories.map((cat)=>(
              <label key={cat.name}>
                <input value={cat.name} onChange={(e)=>toggleFilter(e.target.value,setCategory)} type="checkbox" className='hidden peer' checked={category === cat.name}/>
        ...
        <div className='mt-8'>
            ...
            <div className='grid grid-cols-1 xs:grid-cols-2 md:grid-col-3 lg:grid-cols-4 xl:grid-cols-5 gap-8'>
        ...
        <div className="flexCenter mt-4 pb-10 gap-4">
